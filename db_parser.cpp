...
bool DBParser::parse(string db_filename, DataStore& ds)
{
    ...
    while (!ifile.fail() && !error_) {
        ...
        if (state == FIND_SECTION) {
            ...
            if ((line.size() > 3) && line[0] == '<' && line[1] == '/'
                ...
                if (it != parsers_.end()) {
                    error_ = it->second->parse(sectionText,
                                               ds, startLineNo, errorMsg_);
                    if (error_) {
                        lineno_ = startLineNo;
                    }
                    ...
                    cerr <<
                         "No section parser available for "
    ...
    if (!error_) {
        for (map<string, SectionParser*>::iterator
                it = parsers_.begin();
                it != parsers_.end();
                ++it)
        {
    ...
    std::string& errorMsg)
{
    ...
    is >> password_;
    if (is.fail()) {
        errorMsg = "Unable to read pssword";
        return NULL; 
    }

    return new User(username, balance, type, password_);
}
